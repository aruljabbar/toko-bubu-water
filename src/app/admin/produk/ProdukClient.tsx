...
export default function ProdukClient({ daftarProduk, userRole }: { daftarProduk: any[], userRole: string }) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">📦 Katalog Inventaris Produk</h1>

      {/* FORM TAMBAH PRODUK HANYA UNTUK OWNER */}
      {userRole === 'owner' && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border">
          ...
          <form action={async (formData) => {
            ...
            <div className="grid grid-cols-5 gap-4">
              ...
              <div><label className="block text-xs font-bold text-slate-500 mb-1">Stok Awal</label><input type="number" step="any" name="stok" required className="w-full border rounded-lg p-2 text-sm" /></div>
              <div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-base font-bold text-slate-700">📋 Daftar Detail Produk {userRole === 'kasir' && '(View Only)'}</h2>
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="bg-slate-50 font-bold text-slate-500">
              <tr>
                ...
                {userRole === 'owner' && <th className="px-4 py-3">Modal</th>}
                <th className="px-4 py-3">Harga Jual</th>
                <th className="px-4 py-3 text-center">Stok</th>
                {userRole === 'owner' && <th className="px-4 py-3 text-center">Aksi Manajemen</th>}
              </tr>
            </thead>
                ...
                <tr key={p.id} className="hover:bg-slate-50/50">
                  ...
                  {userRole === 'owner' && (
                    <td className="px-4 py-3">
                      <div className="text-xs text-rose-600 font-bold">Rp {p.hargaModal.toLocaleString('id-ID')}</div>
                    </td>
                  )}
                  
                  <td className="px-4 py-3">
                    <div className="text-sm text-emerald-600 font-black">Rp {p.harga.toLocaleString('id-ID')}</div>
                    {p.hargaGrosir && <div className="text-[10px] text-slate-500 font-bold mt-0.5">Grosir: Rp {p.hargaGrosir.toLocaleString('id-ID')} (Min {p.minGrosir})</div>}
                  </td>
                  <td className="px-4 py-3 text-center font-bold text-base">{p.stok}</td>
                  
                  {userRole === 'owner' && (
                    <td className="px-4 py-3 text-center flex flex-col gap-1">
                      <button onClick={() => { setEditData(p); setIsRestockModalOpen(true); }} className="text-[10px] bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-1 rounded font-bold hover:bg-emerald-100 uppercase">📦 Beli Stok</button>
      ...
      {isRestockModalOpen && editData && userRole === 'owner' && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            ...
            <p className="text-xs text-slate-500 mb-4 font-semibold">Sistem otomatis menghitung Harga Modal rata-rata (Moving Average).</p>
            <form action={async (fd) => { await restockProduk(fd); setIsRestockModalOpen(false); }} className="space-y-4">
              ...
              <div><label className="text-xs font-bold text-slate-700">Jumlah Barang Masuk (Qty)</label><input type="number" step="any" name="qtyMasuk" required className="w-full border-2 border-emerald-200 rounded p-3 text-lg font-bold" /></div>
              <div><label className="text-xs font-bold text-slate-700">Harga Beli Satuan Baru</label><input type="number" name="modalBaru" required defaultValue={editData.hargaModal} className="w-full border-2 border-emerald-200 rounded p-3 text-lg font-bold" /></div>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form action={async (fd) => { await editProduk(fd); setIsEditModalOpen(false); }} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="text-xs font-bold">Satuan</label><input list="admin-satuan-list" name="satuan" defaultValue={editData.satuan} className="w-full border rounded p-2" /></div>
