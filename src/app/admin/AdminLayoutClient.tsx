      ...
      <aside className={`fixed lg:sticky top-0 left-0 h-[100dvh] z-50 w-64 bg-slate-900 text-white p-5 flex flex-col justify-between border-r border-slate-800 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="space-y-6 overflow-y-auto pr-1">
          ...
          <nav className="flex flex-col gap-1.5 pt-4 lg:pt-0">
            ...
            {/* MENU BARU: LAPORAN KASIR */}
            <Link href="/admin/laporan-kasir" onClick={handleLinkClick} className={`px-4 py-2.5 rounded-xl font-bold text-sm transition flex items-center gap-2 ${pathname === '/admin/laporan-kasir' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>📄 Laporan Hari Ini</Link>
